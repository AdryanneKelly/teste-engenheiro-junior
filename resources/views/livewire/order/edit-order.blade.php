        ...
        <div class="container mx-auto">
            <div class="flex flex-col items-center">
                <form wire:submit.prevent='update' class="shadow-sm shadow-white rounded-xl px-10 pb-10 mt-10 mx-2">
                    <h1 class="mt-10 text-white w-auto md:w-[400px] lg:w-[550px] font-medium">Editar pedido</h1>
                    ...
                    @foreach ($orderItems as $index => $orderItem)
                        <div class="flex gap-4 mb-4">
                            ...
                            <button type="button" wire:click="removeProduct({{ $index }})"
                                class="border-2 px-5 rounded-full py-2 mt-4 items-end text-red-500 border-red-500 hover:bg-red-700 hover:bg-opacity-15">Remover</button>
                        </div>
                    @endforeach
                    <div class="flex flex-row w-full justify-center">
                        <button type="button" wire:click="addProduct"
                            class="border-2 px-5 rounded-full py-2 items-end text-sm border-purple-500 text-purple-500 hover:bg-slate-600 mt-4">Adicionar
                            Produto</button>
                    </div>
                    <hr class="mt-10 mb-4 ">
                    <div class="mb-4">
                        <label for="discount" class="block text-sm font-medium text-white">Desconto (R$)</label>
                        <input type="number" wire:model.lazy="discount" step="any" id="discount"
                            wire:change="calculateTotal"
                    ...
                    <div class="flex flex-col-reverse text-center md:flex-row gap-2 mt-5 md:items-center">
                        ...
                        <button type="submit"
                            class="border-2 px-5 rounded-full py-2 items-end text-white hover:bg-slate-600">Salvar
                            alterações</button>
                    </div>
                </form>
            </div>
